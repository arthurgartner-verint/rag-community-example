...
export const loader = async () => {
  ...
  const communityApiKeyExists = process.env.COMMUNITY_API_KEY ? true : false;

  return json({ communityUrl, communityApiKeyExists })
}
...
export const action: ActionFunction = async ({ request }) => {
  ...
  let tokensUsed = 0;
  const communityApiKey = process.env.COMMUNITY_API_KEY;
  const communityUrl = process.env.COMMUNITY_URL;
    ...
    if (keywordExtract) {
      ...
      try {
        const response = await openai.chat.completions.create({
          model: gptModel,
          messages: [{ role: 'user', content: `Extract the most important keywords from this text: "${query}". Only return a csv list of the keywords, no other text.` }],
          temperature: 0.5
        })

        keywords = response.choices[0].message.content?.split(',').map((keyword) => keyword.trim());

      } catch (error) {
        console.error('OpenAI error: ', error);
        return json(['Error processing your request. Failed at state keyword extraction.']);
      }
    }

    var communitySearchResults;

    //Execute query against community search function
    try {
      const response = await fetch(`${communityUrl}/api.ashx/v2/search.json?query=${query}`, {
        headers: {
          'Rest-User-Token': `${communityApiKey}`,
          'Content-Type': 'application/json'
        }
      })

      communitySearchResults = response;
    } catch (error) {
      console.error('OpenAI error: ', error);
      return json(['Error processing your request. Failed at state Community search.']);
    }

    return json({ keywords, gptModel, tokensUsed, communitySearchResults });
  ...
  return (
    <div className="p-[50px] h-[100vh] flex flex-col">
      ...
      <div className="flex space-x-2">
        <div className="w-1/4">
          <ComponentCard title="Current Settings" subtitle="Update your settings via the gear icon on the right" children={<CommunityIntegration />} />
        </div>
        <div className="w-1/4">
          <ComponentCard title="Query Configuration" subtitle="Modify query settings here" children={<QueryConfiguration />} />
        </div>
        <div className="w-2/4">
